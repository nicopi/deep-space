...
%
% (8176,7154) LDPC Code
% MATLAB, Soft decoding, sum-product
%
...
nblocks=2; % Number of blocks to TX
maxIT=10; % Maximum number of iterations
SNRsteps=[3:0.05:3.65 3.7:0.01:3.73 3.731:0.001:6]; %SNR values to test
        ...
        while(I<maxIT && ~exit)
            for c=1:ncheck %checknodes
                for i=1:dp %check nodes inputs
                    ...
                    for j=1:dp
                        if i~=j
                            num=num*tanh(M(c,bitToCheck(c,j))/2);
                        end
                    end
                    E(c,bitToCheck(c,i))=log((1+num)/(1-num)); %E(c,b) storer message from check-node c to bit-node b
                end
            end
